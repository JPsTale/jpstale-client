...
/**
 * **武器色进入曳光的强度**：`0.5` = 从白向武器色混一半（`mix(1, 色表色, 0.5)`）。
 *
 * 为什么是这个数（不是拍脑袋）：原版 `smRender.Color_R += sColors[SMC_R] >> 1` 把**色表色的一半**
 * 加到**当时的渲染色**上，那个渲染色就是场景/昼夜色。把它按"**场景色 ≈ 中灰 0.5**"代入：
 * ```
 * 结果 = 0.5 + 色表色/255/2        // 原式在场景色 0.5 时的值
 * mix(1, 色表色/255, 0.5) = 0.5 + 色表色/255/2   // 本式 —— 逐项相同
 * ```
 * ⇒ 本条是**原式的等价值**（前提写在上面：场景色取中灰），不是"按观感调的"。
 * 我们这条带子没有"场景色"这一项（`uColor` 基准恒为白），所以只能这样表达；
 * 若日后给带子补上场景/昼夜项，应改回"加到场景色上"的原式。
 */
