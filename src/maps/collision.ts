...
export class CollisionMesh {
  ...
  _triangleImact(tri: CollisionTri, sp: number[], ep: number[]): boolean {
    ...
    // C++ 336-340 —— 平面异侧测试
    const c1 = this._smPlaneProduct(p1, p2, p3, sp);
    const c2 = this._smPlaneProduct(p1, p2, p3, ep);
   ...
   */
  _wallBlocked(x: number, y: number, z: number, dx: number, dz: number, bodyWidth: number, bodyHeight: number): boolean {
  ...
  checkNextMove(x: number, y: number, z: number, angle: number, dist: number, bodyWidth = 11): { x: number; y: number; z: number; collision: boolean } {
    ...
    for (let ccnt = 0; ccnt < 3; ccnt++) {
      ...
      return { x: newX, y: h.height, z: newZ, collision: false };
    }
